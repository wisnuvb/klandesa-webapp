...
export default function AdminReferralPage() {
  ...
  const createCode = async () => {
    try {
      ...
      setOk(`Kode ${data?.referralCode?.code || form.code} dibuat`);
      setForm({
        code: '',
        label: '',
        ownerName: '',
      ...
      setError(e instanceof Error ? e.message : 'Gagal membuat kode referral');
    } finally {
      setSaving(false);
    }
  };
      ...
      <Card>
        <CardHeader className="space-y-1">
          ...
          <div className="text-sm text-muted-foreground">
            Link akan berbentuk <span className="font-mono">/tim?ref=KODE</span>
            .
          </div>
        </CardHeader>
        <CardContent className="space-y-3">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <Input
              ...
              placeholder="Komisi, contoh: 10% / Rp500.000"
            />
          </div>
          <Textarea
            value={form.notes}
      ...
      <Card>
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="text-sm text-muted-foreground">Belum ada kode.</div>
          ) : (
                ...
                >
                  <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-3">
                    <div className="min-w-0 space-y-1">
                      ...
                      <div className="text-xs text-muted-foreground">
                        Komisi: {row.commission || '-'} · Event:{' '}
                        {row.eventCount}
                      </div>
                      <div className="text-xs text-muted-foreground">
                        Daftar: {row.actionSummary.register_submit ?? 0} ·
                        Kontak: {row.actionSummary.contact_submit ?? 0} ·
                        WhatsApp: {row.actionSummary.whatsapp_click ?? 0}
                      </div>
                    </div>
                    <div className="flex flex-col sm:flex-row gap-2">
